...
// splits a string without allocating any memory on the heap
int split_string2(const char *str, char separator, size_t number_of_strings, size_t string_length, char buf[number_of_strings][string_length])
{
  size_t len = strlen(str);
  ...
  for (size_t i = 0; i < len; i++)
  {
    ...
    if (cur == separator)
    {
      if (strlen(cur_segment) > 0)
      {
        ...
        if (buf_position >= number_of_strings-1)
        {
          perror("buffer too small to hold strings generated by separation");
          return -1;
        }
        //place string segment into buffer
    ...
    else
    {
      ...
      {
        perror("Insufficient string size to hold split string");
        return -1;
      }
